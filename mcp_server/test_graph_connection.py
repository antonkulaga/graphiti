...
async def add_test_memories():
    ...
    result1 = await call_mcp_tool("add_memory", {
    ...
    result2 = await call_mcp_tool("add_memory", {
    ...
    result3 = await call_mcp_tool("add_memory", {
        "name": "Company Info",
...
async def search_and_verify(group_id: str):
    ...
    print("\nSearching for 'Alice'...")
    try:
        result = await call_mcp_tool("search_nodes", {
            "query": "Alice",
            "group_ids": [group_id],
            "max_nodes": 5
        })
    ...
    try:
        result = await call_mcp_tool("search_facts", {
    ...
    try:
        result = await call_mcp_tool("search_facts", {
            "query": "CloudSync project development",
            "group_ids": [group_id],
            "max_facts": 5
        })
        print(f"  Found: {json.dumps(result, indent=2, default=str)}")
